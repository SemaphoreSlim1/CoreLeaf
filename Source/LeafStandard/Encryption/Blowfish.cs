...
{    
    public class Blowfish : IBlowfish
    {
        ...
        public string ApplyPkcs5Padding(string str)
        {
            var newStr = str;
            var padChars = 8 - (str.Length % 8);
            if (padChars != 0)
            {
                for (var i = 0; i < padChars; i++)
                {
                    newStr += Char.ConvertFromUtf32(padChars);
                }
            }
            return newStr;
        }
        ...
        {
            randomSource = RandomNumberGenerator.Create(); //new RNGCryptoServiceProvider();
